...
| `/admin/resume` | `app/admin/(dashboard)/resume/page.tsx` | Resume management: view current PDF and metadata, publish or unpublish, delete, upload or replace (`features/admin/files/ResumeManager`). Both file managers share `useFileUpload` + `FileDropzone` and `useManagedFileMutations`. |
